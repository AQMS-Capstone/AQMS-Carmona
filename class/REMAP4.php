<?php
  ...
  $sql = "SELECT * FROM MASTER INNER JOIN ELEMENTS ON MASTER.e_id = ELEMENTS.e_id WHERE TIMESTAMP LIKE '%$date_now%' OR TIMESTAMP = '$date_tomorrow' ORDER BY TIMESTAMP ASC";
  ...
  for($i = 0; $i < count($bancalAllDayValues_array); $i++)
  {
    ...
    //{
      switch($bancalAllDayValues_array[$i]->e_id)
      {
          ...
          //$date_value = substr($bancalAllDayValues_array[$i]->timestamp, 0, -9);

          //if($date_now == $date_value)
          //{
  ...
  */
  ...
  for($i = 0; $i < count($bancal_co_values); $i++)
  {
    ...
    /*
    $data_hour_value = substr($bancal_co_values[$i]->timestamp, 11, -6);
    $data_date_value = substr($bancal_co_values[$i]->timestamp, 0, -9);

    echo $bancal_co_values[$i]->timestamp."<br/>";
    ...
    {
      if(count($bancal_co_values) == 24 && $data_date_tomorrow == $data_date_value)
      {
        echo "HAHA";
      }
    }
